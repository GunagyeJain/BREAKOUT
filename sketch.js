...
class Ball {
    ...
    updateRoute() {
        ...
        this.route.y = this.speed * getSin(this.angle)  * -1;
    }
    addBrokenBlock(x, y) {
        var bulundu = false;
...
class Block {
    ...
    tryYourChance(){
        let toBeOrNotToBe = Math.floor(Math.random() * 100);
        let which = Math.floor(Math.random()*7);
        let chanceforSuperBall = Math.floor(Math.random()*3);
        let yesOrNoForThreeBalls = Math.floor(Math.random()*2); 
        if(toBeOrNotToBe<15) {  
            switch(which) {
                case 0:
...
function drawWelcome() {
    drawText("BreakOut", "#d9effc", "50px Saira Stencil One", 100, 275);
}
...
function drawTheEnd() {
    drawText("🎉U WIN!!🎉", "#f46f47", "50px Saira Stencil One", 60, 300);
    drawText("Score " + score, "#ddd", "24px Noto Sans", 125, 350);
}


function drawBoard() {
    drawText("Level "+level + " / 4", "#eee", "16px Noto Sans", 260, 580);
